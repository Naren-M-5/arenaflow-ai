...
class FirebaseService {
  ...
  async initRealtimeData() {
    const arenaRef = ref(this.db, 'arenaData');
  ...
  startFirebaseSimulationPusher() {
    ...
    this.intervalId = setInterval(() => {
      ...
      set(ref(this.db, 'arenaData'), { zones: this.zones, queues: this.queues }).catch(e => {
  ...
  shiftDataMetrics() {
    Object.keys(this.zones).forEach(key => {
      ...
      let newDensity = (this.zones[key].density || 50) + shift;
    ...
    Object.keys(this.queues).forEach(key => {
      ...
      let newWaitTime = (this.queues[key].waitTime || 5) + timeShift;
      newWaitTime = Math.max(0, Math.min(60, newWaitTime));
      let status = 'Open';
