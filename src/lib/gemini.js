...
export async function getRoutingRecommendation(gate, section, contextualCrowdData) {
  if (!isRealSetup) {
    ...
    if (gateInfo.density > 75) {
       return `It looks like ${gate} is quite busy right now (${gateInfo.density}% capacity). We suggest walking over to the adjacent gate for faster entry, then following the inner concourse directly to ${section}.`;
    }
